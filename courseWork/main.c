...
Colour getPixel(PictureBmp* image, uint32_t x, uint32_t y){
	size_t tmpX, tmpY, tmpWidth, tmpHeight;
	uint32_t tmpWidth1, tmpHeight1;
	tmpX = (size_t)x;
	tmpY = (size_t)y;
	tmpWidth1 = image->header.width_px;
	tmpHeight1 = image->header.height_px;
	tmpHeight = (size_t)tmpHeight1;
	tmpWidth = (size_t)tmpWidth1;
	tmpY = tmpHeight-1-tmpY;
	size_t ySet = tmpY * ( tmpWidth * 3 + (4 - (tmpWidth * 3 % 4))%4 );
	size_t xSet = tmpX*3;
...
Colour createColour(char* str){
	...
	if(!strcmp(str, "white")){
		...
		printf("You type wrong colour, colour set to white\n");
		Colour pixel = {255,255,255};
		return pixel;
	}
}
...
int main(int argc, char **argv){
            ...
            case 'M':              
                if(!(strcmp("vertical", optarg))){
					...
					if(!(strcmp("horizontal", optarg))){
						...
						printf("%s - You type wrong axys of symmetry, defolt axys is horizontal\n", optarg);
					}
				}               
                break;
 ...
 switch(flag){
        ...
        case'r':
            if(centr2X >= tmp->header.width_px || centr2Y > (tmp->header.height_px - 1) || centrX >= centr2X || centrY >= centr2Y ){
                   printf("You type wrong coords of reflecting area\n");
                   break;
            }
            else{
        ...
        case'd':
            if(radiusFilled > radius){
                   printf("Width of circle line more than circle radius\n");
                   return 0;
                   break;
            }
            else{
